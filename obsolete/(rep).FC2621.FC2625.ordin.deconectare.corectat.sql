SELECT  x.tert                AS x_tert,
        ...
        x.sef_iasi            AS x_sef_iasi,
        x.sef_tg              AS x_sef_tg,
        x.sef_birou           AS x_sef_birou    
...
(SELECT t.ID AS tert, od.id as idod,
        ...
        persoane_contact_terti (t.id) AS persoane_contact,
        'Dan Repede' AS sef_iasi,
        'Florin Nicuta' AS sef_tg,
        case when u.responsabili_consum in ('DANUT URSACHE','OLGA APACHITEI','FLORIN NICUTA') then 'Florin Nicuta' else 'Dan Repede' end AS sef_birou
