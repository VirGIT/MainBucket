select x.tert as x_tert,
       ...
       case
         when round(sum(x.sold),2) is null then to_char('0')
         else
          replace(to_char(round(sum(x.sold),2)),'.',',')
       end as x_valoare_sold,
       case
	     when round((penx.sold), 2) is null then to_char('0')
         when round((penx.sold), 2) < 1 then
          replace('0'||to_char(round((penx.sold),2)),'.',',')
         else
          replace(to_char(round((penx.sold),2)),'.',',')
       end as x_penalitati,
       ...
       nvl(( select nume from semnaturi_rj where unitier_cod = rajac_pkg.get_ui_rel_cod(x.cod_ui, 'BIR_COM') and sysdate between de_la and nvl(la, sysdate) ),x.cod_ui) nume_birou,
            ...
            ( select foto from semnaturi_rj where matricol = (select distinct matricol from angajati where usercros_nume = user) and sysdate between de_la and nvl(la,sysdate)) semnatura_user
